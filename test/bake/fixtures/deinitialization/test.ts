...
test("rapid start→stop does not race the watcher thread's startup", async () => {
  // Watcher::shutdown previously decided ownership by reading
  // watchloop_handle: false meant "no thread; free here". But there is a
  // window between start() spawning the FileWatcher thread and that thread
  // reaching watchloop_handle.store(true). A shutdown landing in that window
  // freed the struct and the thread then wrote to freed memory (an ASAN
  // heap-use-after-free at thread_main's first store). Loop start→stop→GC
  // tightly so at least one iteration's deinit lands in that window — under
  // ASAN's instrumentation overhead the watcher thread is slow enough to
  // schedule that this is reliable on the x64-asan lane.
  const N = 50;
